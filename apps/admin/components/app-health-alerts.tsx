...
export default function AppHealthAlerts({
  ...
  const lastSuccessfulSync = syncRuns.find((run) => run.status === "SUCCESS") ?? null;

  const lastCompletedSync =
    syncRuns.find((run) => run.status === "SUCCESS" || run.status === "SKIPPED") ?? null;
  ...
  const latestGoogleMetric = metrics.find((item) => item.provider === "GOOGLE") ?? null;
  ...
  if (!lastSuccessfulSync) {
    alerts.push({
      ...
      description: "Bu uygulama için henüz SUCCESS durumunda bir sync run görünmüyor.",
    });
  }

  if (lastCompletedSync) {
    const age = hoursSince(lastCompletedSync.finishedAt || lastCompletedSync.startedAt);

    if (age !== null && age > 24) {
      alerts.push({
        level: "warning",
        title: "Sync data may be stale",
        description: "Son tamamlanan sync 24 saatten daha eski görünüyor.",
      });
    }
  }
  ...
  if (!latestMetric) {
    alerts.push({
      ...
      description: "Bu uygulama için henüz metrics snapshot üretilmemiş görünüyor.",
    });
  } else {
      ...
      <div className="mt-5 space-y-3">
        {alerts.map((alert, index) => (
          ...
          >
            ...
            <div className="mt-2 text-sm leading-7 opacity-90">{alert.description}</div>
          </div>
        ))}
      </div>
    </div>
